...
def get_features_hook0(self, input, output):
    ...
    # return features
    # np.save("./features_all/self_supervised_cifar100/1st_block.npy",features)


def get_features(model, dataloader, Feature, max_images=10 ** 10, verbose=False):
    features, labels = [], []
    total = 0
    ...
    for index, (img, label) in enumerate(dataloader):
        ...
        handle0 = model.module.layer1[1].register_forward_hook(get_features_hook0)
...
def main():
    ...
    features_train, labels_train = get_features(
        model.encoder, train_loader, Feature
    )  # using feature befor MLP-head
    Feature = np.array(Feature)
    print("Feature size is:", Feature.shape)
    np.save("./features_all/self_supervised_cifar100/1st_block.npy",Feature)

    # np.save("./features_all/self_supervised_cifar10/features_train2.npy",features_train)
    # np.save("./features_all/self_supervised_cifar10/labels_train2.npy",labels_train)
    # print("features_train type is:", type(features_train))

    # features_test, _ = get_features(model.encoder, test_loader)
    # np.save("./features_all/self_supervised_cifar10/features_test2.npy",features_test)
    ...
    for d in ds:
        ...
        # np.save("./features_all/self_supervised_cifar10/features_ood2.npy",features_ood)
